    ...
    for eps in range(len(trjs)):
        ...
        for k in range(len(S_space)):
            if Wik[k] == 0 or Wik[k] == len(Ui):  # 没有出现的和第一个出现的都不计入
                ...
                ind = int(Wik[k])
                for j in range(ind + 1, len(Ui)):  # 只计算之后的，自己不算
                    AF[k] = AF[k] + Ci[j]
    AF[0] = 0
    return AF
...
def generate_trjs(n_trj, N, RL, env):
    ...
    for eps in range(n_trj):
        ...
        trjs.append(trj)
    return trjs
def Im_s_AFplusBE(trjs, beforestate,tempn):
    n_state = len(S_space)
    ...
    k = 0  # 如果没有新加值,也就是到了计算终点,我们的k要有返回值否则会报错
    ...
    for i in range(n_state):
        if P[i] > P[k - 1]:
            beforestate[i] = 1
    if k==0:
        print("there is no new extreme point")
    Im[k - 1] = tempn * (tempn + 1) / 2  # 根据目前轨迹，计算每个状态的价值

    return Im, beforestate

def Im_s_OBE(trjs, beforestate,tempn):
    n_state = len(S_space)
    Im = np.zeros((n_state, 1))
    O, index_O, OBE, OAF, P, P2, BE, AF = Im_O_OBE_OAF(trjs)
    C = OBE/O
    ...
    l= len(inds)
    print("index",index_O)
    k=index_O[inds[0]] #极值点所对应的状态
    print("k",k)
    if beforestate[k-1]==0:
        print("not done before")

    # if k==0:
    #     print("there is no new extreme point")
    Im[k] = tempn * (tempn + 1) / 2  # 根据目前轨迹，计算每个状态的价值
...
def Im_s(trjs, beforestate):
    ...
    #     for w in range(len(trjs[i])):
    #         if trjs[i][w]==k and mink>w:
    #             mink=w
    ...
    if k==0:
        print("there is no new extreme point")
    return k,beforestate#,mink
def resetIm(Im):
    print("we need to reset Im")
    return Im

def train(N, RL, env, Im, turns,beforestate1):
    n_state = len(S_space)
    trainnumber = 1000000
    ...
    for eps in range(trainnumber):#直到局部收敛为止
        """
        需要重新定义局部收敛的条件，我们只要得到局部的收敛策略即可
        """
        getstatelogo = np.zeros(n_state)
        ...
        while step < N:
            ...
            # print(state)
            beforestate[state - 1] = 1 #虽然每条轨迹都会记但是只有最后一条,也就是最短路径那条,用上了.
            if Im[state - 1] > 0 and getstatelogo[state - 1] == 0:
                #reward = reward + Im[state - 1] * 0.1 #这个0.1 和下面终止条件中的前10个是对应的
                reward = reward*max(Im) + Im[state - 1] # 这个0.1 和下面终止条件中的前10个是对应的
        ...
        #if (sum(r[eps - 10:eps]) >0.99* sum(Im) and sum(usingstep[eps - 10:eps]) ==  usingstep[eps]* 10):
        steps=steps+step
        if (sum(r[eps - 10:eps]) > 9* sum(Im) and sum(usingstep[eps - 10:eps]) == usingstep[eps] * 10):
            print("this turn have done")
            break


    return beforestate, step,steps
...
def main_MAZE(env):
    ...
    tempn = 0
    strr = 0
    beforestate = np.zeros(n_state)# 之前确认过的点，可以在不同的指标中有不同的用法，在OBE中可以把排序在关键点之前的都叫beforestate
    totalsteps = []
    while tempn < 100:
        tempn +=1
        trjs = generate_trjs(n_trj, N_, RL, env)  # 用现有策略产生轨迹
        # [number, beforestate] = Im_s(trjs, beforestate)
        # if number == 0:
        #     trjs = generate_trjs(n_trj,N,RL,env)
        #     [number, beforestate] = Im_s(trjs, beforestate)
        #Im = np.zeros(n_state)
        # print("")
        # Im[number - 1] = tempn*(tempn+1)/2# 根据目前轨迹，计算每个状态的价值
        [Im, beforestate] = Im_s_AFplusBE(trjs,beforestate,tempn)
        #[Im, beforestate] = Im_s_OBE(trjs, beforestate, tempn)
        print("Im", Im)
        #time.sleep(5)
        # Im[tempn-1]=1
        [beforestate, strr,steps] = train(N, RL, env, Im, tempn,beforestate)
        print('neesstep', strr)  # 根据各个状态的价值提升策略
        N_ = N0+ strr
        N =N1+ strr  # 延长探索步骤
        totalsteps.append(steps)
        print("tempn",tempn," steps ", steps, "totall steps", sum(totalsteps))
