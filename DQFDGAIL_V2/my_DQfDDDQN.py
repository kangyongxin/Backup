...
class DQfDDDQN:
    ...
    def train_Q_network(self, pre_train=False, update=True):
        ...
        if pre_train:
            ...
            self.config.demo_num = len(self.demo_buffer)
            #print(self.config.demo_num)
            if self.config.demo_num >self.config.BATCH_SIZE:
                self.config.demo_num= int(self.config.BATCH_SIZE * len(self.demo_buffer)/(len(self.replay_buffer)+len(self.demo_buffer)))
